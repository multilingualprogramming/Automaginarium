...
Stage 2 makes configuration editable from the browser. The canonical shape is also captured in `examples/schema.json`.
...
The first output channel becomes the next generation state. In the Stage 3 browser adapter, the second channel can influence color when present. The French Multilingual core remains the intended canonical implementation.
